...
void KCompare ( CHAR string1[rmaxf1], CHAR string2[rmaf2], int KEY)
{
char tmp[20];
int result;
...
BOOL f2;
LPCTSTR pF;
hFile = CreateFile (argv[1], GENERICREAD, 0, NULL,
OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
FsLow = GetFileSize (hFile, NULL);
printf("Fs=%d\n",(int)FsLow);
hMap = CreateFileMapping (hFile, NULL, PAGE_READONLY,
0, 0, NULL);
pFile = MapViewîfFile (hMap, FILE_MAP_READ, 0, 0, 0);
pF=pFile;
...
OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL); f2 = ReadFile(hFile2, Buffer2, BUF_SIZE, &nf2, NULL);
